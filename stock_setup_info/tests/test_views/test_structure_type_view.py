...
from stock_setup_info.serializers import IndustrySerializer, StructureTypeSerializer
...
class GetAllViewsTest(BaseViewTest):
	...
	def test_get_all_structure_types(self):
		...
		response = self.client.get(
			reverse("structure_types-list")
		)

		# fetch the data from the db
		expected = StructureType.objects.all()
		serialized = StructureTypeSerializer(expected, many=True)
