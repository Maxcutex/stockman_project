...
class TestPriceListApi(APITestCase):
    def setUp(self):
        ...
        self.price_list = mixer.blend(
            "stock_maintain.models.PriceList", stock=self.stock, price_date=self.p_date1
        )
        self.p_date = datetime(year=2014, month=11, day=15, hour=0, minute=0, second=0)
        PriceListFactory.create_batch(
            20, stock=self.stock, price_date=self.p_date, sec_code=self.stock.stock_code
        )
    ...
    def test_get_price_list_by_date(self):
        ...
        response = self.client.get(
            reverse("pricelist-list"), {"price_date": self.price_list.price_date}
        )
    ...
    def test_get_price_list_by_range(self):
        ...
        price_list = PriceList.objects.filter(
            price_date__gte=start_date, price_date__lte=end_date, stock=self.stock.id
        ).count()
        self.assertEqual(response.status_code, status.HTTP_200_OK)
