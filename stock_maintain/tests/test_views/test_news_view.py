...
from stock_setup_info.factory import StructureFactory, StructureTypeFactory, StockFactory, IndustryFactory, \
    MainSectorFactory, SubSectorFactory
...
class TestNewsApi(APITestCase):
    def setUp(self):
        ...
        #set_trace()
        self.stock = mixer.blend('stock_setup_info.models.Stock', sub_sector=self.sub_sector, industry=self.industry)
        # set_trace()
        # self.stock = Stock.objects.create(name="name, body=body,", stock_code="asf", structure=self.structure, industry=self.industry)
    ...
    def test_check_featured_news_count(self):
        """
		This test ensures that news that are featured count are ok
		"""
    ...
    def test_get_news_by_section_with_valid_data(self):
        ...
        news_by_section = News.objects.filter(category_news__section_category__section_name__in=search_array)
