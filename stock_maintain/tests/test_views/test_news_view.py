...
from stock_maintain.factory import (
    NewsFactory,
    NewsCategorySectionFactory,
    SectionGroupFactory,
)
...
class TestNewsApi(APITestCase):
    ...
    def test_check_featured_news_count(self):
        ...
        response = self.client.get(reverse("news-list"), {"is_featured": True})
    ...
    def test_get_news_by_section_with_valid_data(self):
        ...
        news_by_section = News.objects.filter(
            category_news__section_category__section_name__in=search_array
        )
    ...
    def test_news_list_by_date(self):
        ...
        # hit the api endpoint
        response = self.client.get(
            reverse("news-list"), {"news_date": self.news.news_date}
        )
    ...
    def test_news_list_by_date_range(self):
        ...
        news_list = News.objects.filter(
            news_date__gte=start_date, news_date__lte=end_date
        ).count()
        self.assertEqual(response.status_code, status.HTTP_200_OK)
