...
class TestAnalysisApi(APITestCase):
    ...
    def test_view_analysis_existing(self):
        ...
        AnalysisCategorySectionFactory(
            analysis=analysis_for_section, section_category=analysis_section
        )
    ...
    def test_analysis_list_by_date(self):
        ...
        # hit the api endpoint
        response = self.client.get(
            reverse("analysis-list"), {"opinion_date": self.analysis.opinion_date}
        )
        analysis_list = AnalysisOpinion.objects.filter(
